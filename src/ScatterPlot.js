...
const ScatterPlot = (props) => {
    ...
    const paint = () => {
        ...
        const dotScaleX = settings.dots.width / (props.dataset.data.length + 1 - settings.dots.ratio);
        const dotScaleY = settings.dots.height / (settings.values.max - settings.values.min);
        ...
        svg
            ...
            .attr("x2", settings.values.x + settings.values.width + settings.dots.width - settings.lines.margin )
            .attr("y1", (item, index) => {
                return settings.labels.y - index * settings.dots.height / linesStep;
            })
            .attr("y2", (item, index) => {
                return settings.labels.y - index * settings.dots.height / linesStep;
            });
        ...
        svg
            ...
            .append("circle")
            .attr("cx", (item , index) => {
                return settings.dots.x + (1 - settings.dots.ratio + index) * dotScaleX;
            })
            // Sets the dot position
            // If the value is at min or below, the bottom of the dot area is selected
            .attr("cy", (item) => {
                let height = (Object.values(item)[1] - settings.values.min) * dotScaleY;
                if (height <= 0 ) {
                    height = 0;
                }
                return settings.dots.y + settings.dots.height - height;
            })
            .attr("r", settings.dots.ratio * dotScaleX)
            // Sets the value of the dataPointIndex attribute
            .attr("dataPointIndex", (item , index) => {
                return index;
            })
            // Sets the color of a dot based on the selection status, red if selected, otherwise dodgerblue
            .style("fill", (item , index) => {
                return props.selection != null && props.selection.includes(index) ? "red" : "dodgerblue";
            })
            // Invokes the onSelect handle when a dot is clicked on
            .on("click", (e) => {
            ...
            // When the pointer enters a dot area, the tooltip becomes visible (after setting its text and position)
            .on("mouseenter", (e) => {
                setTooltip(tooltip, parseInt(e.target.attributes.dataPointIndex.value));
                transformTooltip(tooltip, d3.pointer(e));
                tooltip.style("visibility", "visible");
            })
            // Moves the tooltip based on the pointer position
            .on("mousemove", (e) => {
                transformTooltip(tooltip, d3.pointer(e));
            })
        ...
        svg
            ...
            .append("text")
            .attr("x", (item , index) => {
                return settings.labels.x + (1 - settings.dots.ratio + index + settings.dots.ratio / 2) * dotScaleX;
            })
        ...
        svg
            ...
            .attr("y", (item, index) => {
                return settings.values.y + settings.values.height - settings.values.baseline - index * settings.dots.height / ((settings.values.max - settings.values.min) / settings.values.step);
            })
            .text((item) => {
